...
#ifndef SAF_SETTING_SQRT_ALGORITHM
  /** Chooses which algorithm will be used by SAF_sqrt function. Values returned
  by the function won't be affected, only performance. Possible values are: 0
  (default, faster, more complex), 1 (binary search), 2 (doubling/halving
  algorithm, slower for high x). */
  #define SAF_SETTING_SQRT_ALGORITHM 0
#endif

#ifndef SAF_SETTING_RANDOM_ALGORITHM
  /** Chooses which algorithm to use for the SAF_random function. This WILL
  affect the output of the function and the random generator's period. Possible
  values are: 0 (default, extremely simple, period 256), 1 (32bit LCG, much
  better, very long period). */
  #define SAF_SETTING_RANDOM_ALGORITHM 0
#endif

#ifndef SAF_SETTING_COS_ALGORITHM
  /** Chooses which algorithm to use for the SAF_cos and SAF_sin functions. This
  won't affect the values returned by the functions, only performance. Possible
  values are: 0 (lookup table, faster but takes some space), 1 (polynomial
  approximation, slower but smaller). */
  #define SAF_SETTING_COS_ALGORITHM 0
#endif

// removes usleep warning
#if (defined SAF_PLATFORM_X11 || defined SAF_PLATFORM_SDL2)
...
#define SAF_VERSION_STRING "1.02"
...
/** Returns a simple pseudorandom number. The number sequence will be the same
  in each program run. SAF_randomSeed() can be called to seed this pseudorandom
  generator. See also SAF_SETTING_RANDOM_ALGORITHM. */
...
/** Computes integer square root of a number. See also
  SAF_SETTING_SQRT_ALGORITHM. */
...
/** Converts given 332 color to amount of red, green and blue. This conversion
  ...
  true gray. */
void SAF_colorToRGB(uint8_t colorIndex, uint8_t *red, uint8_t *green,
  uint8_t *blue);
...
const uint8_t *SAF_FE_GF_getScreenPointer(void);
...
void SAF_FE_GF_init(uint8_t CLIParameters[128])
{
  ...
  if (CLIParameters['u'] != 0)
  {
    ...
    _SDL_upscaleScreen =
      (uint8_t *) malloc(SAF_SCREEN_WIDTH * SAF_SCREEN_WIDTH * 4);
  }
...
void SAF_FE_GF_init(uint8_t CLIParameters[128])
{
  ...
  sfRenderWindow_setPosition(_CSFML_window,winPos);
  ...
  if (screenScale != 0)
  {
    ...
    winSize.y = winSize.x;
    sfRenderWindow_setSize(_CSFML_window,winSize);
  }
...
uint16_t _pokittoAxisThreshold1, _pokittoAxisThreshold2;
uint16_t _pokittoRumbleCooldown = 0;
#endif

uint8_t *_pokittoScreen;
...
uint8_t _pokittoScreenBuffer[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT];
#endif
...
void pokittoOnTimer(void) // for sound
{
...
{
#if SAF_SETTING_POKITTO_JOYHAT
  _pokittoScreen[x * SAF_SCREEN_WIDTH + (SAF_SCREEN_HEIGHT - 1 - y)] = color;
#else
  _pokittoScreen[y * SAF_SCREEN_WIDTH + x] = color;
#endif
}
...
void SAF_FE_playSound(uint8_t sound)
{
  ...
  #if SAF_SETTING_POKITTO_JOYHAT
  if (sound == SAF_SOUND_BOOM && _pokittoRumbleCooldown == 0)
  {     
    _pokittoJoy.Rumble(0.025);
    _pokittoRumbleCooldown = 32;
  }
  #endif
#else
  _SAF_UNUSED(sound);
...
{
#if SAF_SETTING_ENABLE_SAVES
  _pokittoSave.data[index] = data;
  _pokittoSave.saveCookie();
...
{
#if SAF_SETTING_ENABLE_SAVES
  return _pokittoSave.data[index];
#else
  _SAF_UNUSED(index);
  return 0;
#endif
...
uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  switch (button)
  {
    ...
    case SAF_BUTTON_UP: return Pokitto::Core::rightBtn() || 
      (_pokittoJoy.JoyX() < _pokittoAxisThreshold1);
      break;

    case SAF_BUTTON_RIGHT: return Pokitto::Core::downBtn() || 
      (_pokittoJoy.JoyY() > _pokittoAxisThreshold2);
      break;

    case SAF_BUTTON_DOWN: return Pokitto::Core::leftBtn() ||
      (_pokittoJoy.JoyX() > _pokittoAxisThreshold2);
      break;

    case SAF_BUTTON_LEFT: return Pokitto::Core::upBtn() ||
      (_pokittoJoy.JoyY() < _pokittoAxisThreshold1);
      break;

    case SAF_BUTTON_A: 
      return Pokitto::Core::aBtn() || _pokittoJoy.Button1(); break;

    case SAF_BUTTON_B:
      return Pokitto::Core::bBtn() || _pokittoJoy.Button2(); break;
...
int main(void)
{
#if SAF_SETTING_ENABLE_SAVES
  _pokittoSave.begin(
    "SAF" SAF_PROGRAM_NAME,sizeof(_pokittoSave),(char*) &_pokittoSave);
#endif
...
#if CUSTOM_SCREEN_BUFFER
  _pokittoScreen = _pokittoScreenBuffer;
#else
  _pokittoScreen = Pokitto::Display::screenbuffer;
#endif
...
#if SAF_SETTING_POKITTO_JOYHAT
  _pokittoAxisThreshold1 = _pokittoJoy.joyScale / 4;
  _pokittoAxisThreshold2 = _pokittoJoy.joyScale - _pokittoAxisThreshold1;
#endif
  ...
  while (Pokitto::Core::isRunning())
  {
...
#if SAF_SETTING_POKITTO_JOYHAT
    if (_pokittoRumbleCooldown > 0)
      _pokittoRumbleCooldown--;
#endif
    ...
    if (time >= nextFrame)
    {
      ...
      const uint8_t *p = _pokittoScreen;
  ...
  #define SCR_X 0
      ...
      for (int16_t j = 0; j < 176; ++j)
      {
        ...
        {
          const uint8_t *l = _pokittoScreen + SAF_SCREEN_WIDTH * upscaleMapRow;
...
uint8_t _ncScreen[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT];
uint8_t _ncButtonStates[SAF_BUTTONS];

uint8_t _ncCurrentSound = 0;
uint32_t _ncSoundEnd = 0;

uint32_t _ncGetTime(void)
{
  struct timeval now;
...
void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  _ncScreen[y * SAF_SCREEN_WIDTH + x] = color;
}

void SAF_FE_playSound(uint8_t sound)
{
  _ncCurrentSound = sound;
  _ncSoundEnd = _ncGetTime() + 1000;
}

uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  return _ncButtonStates[button];
}
...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    {
      case KEY_UP: case 'w':
        _ncButtonStates[SAF_BUTTON_UP] = 1; break;
 
      case KEY_LEFT: case 'a':
        _ncButtonStates[SAF_BUTTON_LEFT] = 1; break;

      case KEY_RIGHT: case 'd':
        _ncButtonStates[SAF_BUTTON_RIGHT] = 1; break;

      case KEY_DOWN: case 's':
        _ncButtonStates[SAF_BUTTON_DOWN] = 1; break;

      case ' ': case 'y': case 'z': case 'j':
        _ncButtonStates[SAF_BUTTON_A] = 1; break;

      case 'x': case 'k':
        _ncButtonStates[SAF_BUTTON_B] = 1; break;

      case 'c': case 'l':
        _ncButtonStates[SAF_BUTTON_C] = 1; break;
    ...
    const uint8_t* scr = _ncScreen;
    const uint8_t* scr2 = _ncScreen + SAF_SCREEN_WIDTH;
...
#if SAF_SETTING_ENABLE_SOUND
BeepPin1 _arduboyBeep;
#endif

void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  _arduboy.drawPixel(32 + x,y,color ? WHITE : BLACK);
}
...
void SAF_FE_playSound(uint8_t sound)
{
  ...
  _arduboyBeep.tone(_arduboyBeep.freq(f),t);
#endif
}
...
uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  ...
  return _arduboy.pressed(button);
}
...
void setup(void)
{
  ...
  for (uint8_t y = 0; y < 64; ++y)
    for (uint8_t x = 0; x < 128; ++x)
      _arduboy.drawPixel(x,y,c);

  _arduboy.setFrameRate(SAF_FPS);
  SAF_FE_init();
...
uint8_t _espboySaveValidValue = 0;
EEPROMClass _espboyEeprom;
#endif

#define MCP23017address 0
#define MCP4725address  0x60

Adafruit_MCP23017 _espboyMcp;
Adafruit_MCP4725 _espboyDac;
TFT_eSPI _espboyTft;
uint8_t _espboyScreen[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT];
uint8_t _espboyKeys = 0;

PROGMEM uint16_t _espboyPalette[256] =
{
  SAF_FE_PALETTE_565
};
...
void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  _espboyScreen[y * SAF_SCREEN_WIDTH + x] = color;
}
...
#if SAF_SETTING_ENABLE_SAVES
void _espboyCheckEeprom(void)
{
  if (_espboyEeprom.read(0) == _espboySaveValidValue)
    return;

  _espboyEeprom.write(0,_espboySaveValidValue);

  for (uint8_t i = 0; i < SAF_SAVE_SIZE; ++i)
    _espboyEeprom.write(i + 1,0);

  _espboyEeprom.commit();
}
#endif
...
#if SAF_SETTING_ENABLE_SAVES
  _espboyCheckEeprom();
  _espboyEeprom.write(index + 1,data);
  _espboyEeprom.commit();
#else
  _SAF_UNUSED(index);
  _SAF_UNUSED(data);
...
#if SAF_SETTING_ENABLE_SAVES
  _espboyCheckEeprom();
  return _espboyEeprom.read(index + 1);
#else
  _SAF_UNUSED(index);
#endif
}
...
uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  switch (button)
  {
    case SAF_BUTTON_UP:    return _espboyKeys & 0x02; break;
    case SAF_BUTTON_DOWN:  return _espboyKeys & 0x04; break;
    case SAF_BUTTON_RIGHT: return _espboyKeys & 0x08; break;
    case SAF_BUTTON_LEFT:  return _espboyKeys & 0x01; break;
    case SAF_BUTTON_A:     return _espboyKeys & 0x10; break;
    case SAF_BUTTON_B:     return _espboyKeys & 0x20; break;
    case SAF_BUTTON_C:     return _espboyKeys & 0x80; break;
    default: return 0; break;
  }
}
...
void setup(void)
{ 
  _espboyDac.begin(MCP4725address);
  delay(100);
  _espboyDac.setVoltage(0,false);
  _espboyMcp.begin(MCP23017address);
  delay(100);

#if SAF_SETTING_ENABLE_SAVES 
  _espboySaveValidValue = SAF_FE_hashStr(SAF_PROGRAM_NAME);
#endif

  // buttons
  for (uint8_t i = 0; i < 8; i++)
  {
    _espboyMcp.pinMode(i,INPUT);
    _espboyMcp.pullUp(i,HIGH);
  }

  _espboyMcp.pinMode(8,OUTPUT);
  _espboyMcp.digitalWrite(8,LOW);
  
  _espboyTft.begin();
  delay(100);
  _espboyTft.setRotation(0);

  _espboyTft.setAddrWindow(0,128,0,128);
  _espboyTft.fillScreen(TFT_BLACK);

  _espboyDac.setVoltage(4095,true); // backlight
...
#if SAF_SETTING_ENABLE_SAVES
  _espboyEeprom.begin(SAF_SAVE_SIZE + 1);
#endif
...
void loop(void)
{
  ...
  _espboyKeys = ~_espboyMcp.readGPIOAB() & 255;

  const uint8_t *pixel = _espboyScreen;
  ...
  for (int y = 0; y < SAF_SCREEN_HEIGHT; ++y)
  {
    ...
    for (int x = 0; x < SAF_SCREEN_WIDTH; ++x)
    {
      uint16_t c = pgm_read_word(_espboyPalette + *pixel);
      *p = c;
      p++;
      *p = c;
      p++;
    ...
    _espboyTft.pushColors(line,SAF_SCREEN_WIDTH * 2);
    _espboyTft.pushColors(line,SAF_SCREEN_WIDTH * 2);
  }
}
...
Display *_nibbleDisplay;
uint16_t *_nibbleFrameBuffer;
uint8_t _nibbleButtons[SAF_BUTTONS];

PROGMEM uint16_t _nibblePalette[256] =
{
  SAF_FE_PALETTE_565
};
...
void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  uint16_t c = pgm_read_word(_nibblePalette + color);

  c = ((c << 8) | (c >> 8)); // endien, TODO: could rather switch in the palette

  uint16_t *p = _nibbleFrameBuffer + y * SAF_SCREEN_WIDTH * 4 + x * 2;
...
#define cbf(b,n)\
  void b ## _down() { _nibbleButtons[n] = 255; }\
  void b ## _up() { _nibbleButtons[n] = 0; }
...
{
  Nibble.begin();
  _nibbleDisplay = Nibble.getDisplay();
  _nibbleFrameBuffer = 
    (uint16_t *) _nibbleDisplay->getBaseSprite()->frameBuffer(0);
...
#if SAF_SETTING_ENABLE_SAVES
const Gamebuino_Meta::SaveDefault _metaSaveDefault[] =
  { { 0, SAVETYPE_BLOB, SAF_SAVE_SIZE, 0 } };
#endif

uint8_t _metaLEDCountdown = 0;

PROGMEM uint16_t _gbmPalette[256] =
{
  SAF_FE_PALETTE_565
};
...
void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  gb.display.drawPixel(8 + x,y,(Color) pgm_read_word(_gbmPalette + color));
}
...
void setup(void)
{
  ...
  for (uint8_t y = 0; y < 64; ++y)
    for (uint8_t x = 0; x < 80; ++x)
      gb.display.drawPixel(x,y,(Color) pgm_read_word(_gbmPalette + 
        SAF_SETTING_BACKGROUND_COLOR));

#if SAF_SETTING_ENABLE_SAVES
  gb.save.config(_metaSaveDefault);
#endif
...
#if SAF_SETTING_ENABLE_SOUND
Oscillator *_ringoOsc;
#endif

uint8_t _ringoArrows;

//uint8_t ringoScreen[SAF_SCREEN_WIDTH * SAF_SCREEN_HEIGHT];

void SAF_FE_drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
  _mp.display.fillRect(16 + x * 2,y * 2,2,2,_ringoPalette[color]);
}
...
void SAF_FE_playSound(uint8_t sound)
{
  ...
  _ringoOsc->setWaveform(wave);
  _ringoOsc->beep(freq,dur);
#else
  _SAF_UNUSED(sound);
#endif
}
...
uint8_t SAF_FE_buttonPressed(uint8_t button)
{
  switch (button)
  {
    #define b(but) (_mp.buttons.timeHeld(but) > 0)
    #define r(but) return b(but); break;

    case SAF_BUTTON_UP: return (_ringoArrows & 0x04) || b(BTN_2); break;
    case SAF_BUTTON_DOWN: return (_ringoArrows & 0x08) || b(BTN_5); break;
    case SAF_BUTTON_RIGHT: return (_ringoArrows & 0x01) || b(BTN_6); break;
    case SAF_BUTTON_LEFT: return (_ringoArrows & 0x02) || b(BTN_4); break;
...
#if SAF_SETTING_ENABLE_SOUND
  _ringoOsc = new Oscillator(SINE);
  addOscillator(_ringoOsc);
  _ringoOsc->setVolume(64);
#endif
...
void loop(void)
{
  ...
  _mp.display.fillRect(
    0,0,LCDWIDTH,LCDHEIGHT,_ringoPalette[SAF_SETTING_BACKGROUND_COLOR]);
  ...
  _ringoArrows = 0x00 | 
    ((_mp.buttons.getJoystickX() < 200)) << 0 |
    ((_mp.buttons.getJoystickX() > 900)) << 1 |
    ((_mp.buttons.getJoystickY() < 200)) << 2 |
    ((_mp.buttons.getJoystickY() > 900)) << 3;
...
void _SAF_FE_loadSaveMemory(void)
{
#ifdef __EMSCRIPTEN__
...
#if SAF_SETTING_RANDOM_ALGORITHM == 1
  uint32_t
#else
  uint8_t
#endif
  _SAF_currentRandom = 0;
...
#if SAF_SETTING_COS_ALGORITHM == 0
_SAF_CONST int8_t _SAF_cosTable[64] =
{
  127,126,126,126,125,125,125,125,123,123,122,122,120,119,119,118,116,115,114,
  112,111,109,108,106,104,102,100,99,96,94,92,90,88,85,83,81,78,76,73,71,68,65,
  63,60,57,54,51,49,46,43,40,37,34,31,28,24,21,18,15,12,9,6,3,0
};
#endif
...
int8_t SAF_cos(uint8_t phase)
{
#if SAF_SETTING_COS_ALGORITHM == 1
  return SAF_sin(phase + 64);
#else
...
int8_t SAF_sin(uint8_t phase)
{
  ...
  phase %= 128; // small correction for border values
  result += 2 * (phase == 63 || phase == 64);
...
{
#if SAF_SETTING_RANDOM_ALGORITHM == 1
  _SAF_currentRandom = 32310901 * _SAF_currentRandom + 37;
  return _SAF_currentRandom >> 16;
#else
...
uint16_t SAF_sqrt(uint32_t number)
{
#if SAF_SETTING_SQRT_ALGORITHM == 1
  uint32_t m, l = 0, r = number < 8300000 ? number / 128 + 40 : 65535;
  ...
  return r;
#elif SAF_SETTING_SQRT_ALGORITHM == 2
  uint32_t a = 1, b = number;
  ...
  while (a * a > number) // iterate to exact value
    a--;
...
uint8_t _SAF_getNextImagePixel(void)
{
  uint8_t result = 0;
...
void _SAF_reloadSaveMemory(void)
{
