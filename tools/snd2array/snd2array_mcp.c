...
static char *convert_to_string(const Snd2ArrayConfig *cfg, Snd2ArrayResult *out_result)
{
  FILE *tmp = tmpfile();
  char *text;
  ...
  if (tmp == NULL)
  {
    *out_result = (Snd2ArrayResult){0, 1, "could not create temporary output"};
    return NULL;
  }

  *out_result = snd2array_convert(cfg, tmp);
  ...
  if (text == NULL)
    *out_result = (Snd2ArrayResult){0, 1, "could not read temporary output"};
...
static void respond_initialize(const char *id)
{
  printf("{\"jsonrpc\":\"2.0\",\"id\":%s,\"result\":{"
    ...
    "\"serverInfo\":{\"name\":\"snd2array\",\"version\":\"0.1.0\"},"
    "\"capabilities\":{\"tools\":{}}}}\n", id);
  fflush(stdout);
}
...
static void respond_tools_list(const char *id)
{
  printf("{\"jsonrpc\":\"2.0\",\"id\":%s,\"result\":{\"tools\":[{"
    "\"name\":\"snd2array_convert\","
    "\"description\":\"Convert raw mono 8kHz 8bit PCM to 4-bit packed C array.\","
  ...
  const jsmntok_t *tokens, int root)
{
  ...
  if (!json_get_string(line, tokens, paramsToken, "name", toolName, sizeof(toolName)) ||
    strcmp(toolName, "snd2array_convert") != 0)
  {
